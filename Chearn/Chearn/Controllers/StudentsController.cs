...
namespace Chearn.Controllers
{
    public class StudentsController : Controller
    {
        ...
        public async Task<JsonResult> GetStudentStatistics(int studentID, int courseID)
        {
            var student = db.Students.Find(studentID);
            var cUser = db.CUsers.Find(student.CUserID);
            var studentReviews = await ReviewController.GetStudentReviews(studentID, courseID);
            var course = db.Courses.Find(courseID);
            var studentStats = new IndividualStatisticsViewModel();
            studentStats.ID = studentID;
            studentStats.FirstName = cUser.FirstName;
            studentStats.LastName = cUser.LastName;
            studentStats.Email = cUser.Email;
            studentStats.StartDate = studentReviews.Min(r => r.TimeStamp);
            studentStats.AverageReviewsPerDay = studentReviews.Count() / ((double)(DateTime.Now - studentStats.StartDate).Days < 1 ? 1 : (double)(DateTime.Now - studentStats.StartDate).Days);
            studentStats.AverageNewLessonsPerDay = studentReviews.GroupBy(r => r.QuestionID).Count() / ((double)(DateTime.Now - studentStats.StartDate).Days < 1 ? 1 : (double)(DateTime.Now - studentStats.StartDate).Days);
            studentStats.ProjectedCompletionDate = DateTime.Now.AddDays((course.Lessons.Count() - studentReviews.GroupBy(r => r.QuestionID).Count()) / studentStats.AverageNewLessonsPerDay);
            studentStats.FormattedStartDate = studentStats.StartDate.ToString("MM/dd/yyyy");
            studentStats.FormattedProjectedCompletionDate = studentStats.ProjectedCompletionDate.ToString("MM/dd/yyyy");
            return Json(studentStats, JsonRequestBehavior.AllowGet);
        }
